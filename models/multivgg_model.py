...
class MultivggModel(BaseModel):
    ...
    def modify_commandline_options(parser, is_train=True):
        ...
        Returns:
            the modified parser.
        """
        parser.set_defaults(netG='multi_vgg_2x_6blocks', netD='multi_n_layers', n_layers_D=4, 
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            self.model_names = ['G_vgg', 'D']
        else:
            self.model_names = ['G_vgg']
        # define networks; you can use opt.isTrain to specify different behaviors for training and test.
        self.style_layers = ['r11', 'r21', 'r31', 'r41', 'r51']
        self.G_code_layer = ['r32']
        ...
        self.netG_vgg = networks.define_G(opt.input_nc, opt.output_nc, opt.ngf, opt.netG, 
        ...
        if self.isTrain:  # only defined during training time
            ...
            self.optimizer_G = torch.optim.Adam(self.netG_vgg.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D = torch.optim.Adam(self.netD.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizers = [self.optimizer_G, self.optimizer_D]
        
        # show the network structure
        networks.print_network_structure(self.netG_vgg)
        if self.isTrain:
    ...
    def forward(self):
        ...
        content_code = self.vgg(self.real_content, self.G_code_layer)
        style_code = self.vgg(self.real_style, self.G_code_layer)
        self.fake_ref = self.netG_vgg(content_code[0], style_code[0])  # generate output image given the input data_A
